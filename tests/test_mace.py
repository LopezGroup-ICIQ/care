...
class TestEvaluator(unittest.TestCase):
    ...
    def setUpClass(cls):
        try:
            from care.evaluators.mace import MACEevaluator
            cls.model_inter = MACEevaluator(size="small", num_configs=2, max_steps=2)
        except ImportError:
            pytest.skip("MACE not installed, skipping these tests.")
    ...
    def test_parallel_eval(self):
        ...
        def f(inter):
            if '_worker_model' not in globals():
                from care.evaluators.mace import MACEevaluator
                globals()['_worker_model'] = MACEevaluator(size="small", num_configs=2, max_steps=2)
