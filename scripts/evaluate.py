...
from care import MODEL_PATH, ReactionNetwork
from care.gnn import load_model


def main():
    argparser = argparse.ArgumentParser(
        description="Evaluate energy of the adsorbed species with GAME-Net UQ."
    )
    argparser.add_argument(
        "-i",
        type=str,
        dest="i",
        help="Path to the folder containing the ads_intermediates.pkl file",
    )
    argparser.add_argument(
        "-ncores",
        type=int,
        ...
        if intermediate.phase == "gas":
            ...
            with no_grad():
                for i, config in enumerate(
                    intermediate.ads_configs.values()
                ):  # Configuration
                    y = model(config["pyg"])  # unitless
                    config["mu"] = (
                        y.mean * model.y_scale_params["std"]
                        + model.y_scale_params["mean"]
                    ).item()  # eV
    ...
    with no_grad():
        for reaction in crn.reactions:
            ...
            if "-" in reaction.r_type:
                ...
                reaction.e_ts = (
                    y.mean.item() * model.y_scale_params["std"]
                    + model.y_scale_params["mean"],
                    y.scale.item() * model.y_scale_params["std"],
                )
