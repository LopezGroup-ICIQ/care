...
def create_model_report(model_name: str,
    ...
    if config_file_path != None:
        ...
        os.system("cp {} {}/{}/config.toml".format(config_file_path, model_path, model_name))
    ...
    torch.save(model, "{}/{}/model.pkl".format(model_path, model_name))             # Save model as pkl
    torch.save(model.state_dict(), "{}/{}/params.pkl".format(model_path, model_name))  # Save model parameters as pkl
    torch.save(one_hot_encoder_elements, "{}/{}/one_hot_encoder_elements.pth".format(model_path, model_name))  # Save one hot encoder elements    
    ...
    ARC = summary(model, (train_loader.dataset[0]), max_depth=5)
    file1.write(ARC)
    file1.write("---------------------------------------------------------\n")
...
def create_model_report_uq(model_name: str,
                        model_path: str,
                        configuration_dict: dict,
                        model: torch.nn.Module,
                        loaders: tuple[DataLoader],                     
    ...
    formula, family, surface, y_true, y_mean, y_std, y_min, y_max, in_interval, error = [], [], [], [], [], [], [], [], [], []
    for graph in test_loader.dataset:
        formula.append(graph.formula)
        family.append(graph.family)
        surface.append(graph.facet)
        y_true.append(graph.target.numpy()[0])
        y_mean.append(model(graph).mean.cpu().detach().numpy()[0] * model.scaling_params["std"] + model.scaling_params["mean"])
        y_std.append(model(graph).stddev.cpu().detach().numpy()[0] * model.scaling_params["std"])
