...
def create_model_report(input_file: str, 
    ...
    # Save input file provided as first function argument
    os.system("cp {} {}/{}".format(input_file, model_path, model_name))
    
    # Save dataloaders for future use (train/val)
    ...
    w_min, w_max = [], []    # Test set
    x_pred, x_true = [], []  # Train set
    x_min, x_max = [], []    # Train set
    a_pred, a_true = [], []  # Validation set
    a_min, a_max = [], []    # Validation set
    
    for batch in test_loader:
        batch = batch.to("cpu")
        u = model(batch).mean
        std = model(batch).stddev
    ...
    y_pred_min = [w_min[i].item()*std_tv + mean_tv for i in range(N_test)]
    y_pred_max = [w_max[i].item()*std_tv + mean_tv for i in range(N_test)]
    ...
    z_pred_min = [x_min[i].item()*std_tv + mean_tv for i in range(N_train)]
    z_pred_max = [x_max[i].item()*std_tv + mean_tv for i in range(N_train)]
    ...
    b_pred_min = [a_min[i].item()*std_tv + mean_tv for i in range(N_val)]
    b_pred_max = [a_max[i].item()*std_tv + mean_tv for i in range(N_val)]
    ...
    with open("{}/{}/test_set.csv".format(model_path, model_name), "w") as file4:
        ...
        for i in range(len(y_true)):
            if y_true[i] >= y_pred_min[i] and y_true[i] <= y_pred_max[i]:
                within_interval.append("Yes")
            else:
                within_interval.append("No")
        writer.writerow(["System", "Family", "True [eV]", "Prediction [eV]", "Error [eV]", "Abs. error [eV]", "Lower bound [eV]", "Upper bound [eV]", "in_interval"])
        writer.writerows(zip(test_label_list, test_family_list, y_true, y_pred, error_test, abs_error_test, y_pred_min, y_pred_max, within_interval))    
    with open("{}/{}/train_set.csv".format(model_path, model_name), "w") as file4:
        writer = csv.writer(file4, delimiter='\t')
        within_interval = []
        for i in range(len(z_true)):
            if z_true[i] >= z_pred_min[i] and z_true[i] <= z_pred_max[i]:
                within_interval.append("Yes")
            else:
                within_interval.append("No")
        writer.writerow(["System", "Family", "True [eV]", "Prediction [eV]", "Error [eV]", "Abs. error [eV]", "Lower bound [eV]", "Upper bound [eV]", "in_interval"])
        writer.writerows(zip(train_label_list, train_family_list, z_true, z_pred, error_train, abs_error_train, z_pred_min, z_pred_max, within_interval))    
    with open("{}/{}/validation_set.csv".format(model_path, model_name), "w") as file4:
        writer = csv.writer(file4, delimiter='\t')
        within_interval = []
        for i in range(len(b_true)):
            if b_true[i] >= b_pred_min[i] and b_true[i] <= b_pred_max[i]:
                within_interval.append("Yes")
            else:
                within_interval.append("No")
        writer.writerow(["System", "Family", "True [eV]", "Prediction [eV]", "Error [eV]", "Abs. error [eV]", "Lower bound [eV]", "Upper bound [eV]", "in_interval"])
        writer.writerows(zip(val_label_list, val_family_list, b_true, b_pred, error_val, abs_error_val, b_pred_min, b_pred_max, within_interval))
    return "Model saved in {}/{}".format(model_path, model_name)