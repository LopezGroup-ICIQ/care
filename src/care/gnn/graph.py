...
def detect_broken_bond(
    atoms: Atoms, adsorbate_elems: list[str], tol: float
) -> tuple[int]:
    """
    ...
    neighbour_list = [
        ...
        if pair[0] in adsorbate_indices and pair[1] in adsorbate_indices
    ]
    graph = Graph()
    ...
    if len(components) != 2:
        print(
            f"{atoms.get_chemical_formula()}: Found {len(components)} components instead of 2 in the adsorbate graph.\n"
        )
        return None
    else:
        dist_dict = {}
...
def atoms_to_pyggraph(
    ...
    nxgraph, surface_neighbors, bb_idxs = atoms_to_nxgraph(
    ...
    edge_tails_heads = [
        (nodes_list.index(edge[0]), nodes_list.index(edge[1])) for edge in nxgraph.edges
    ]
    ...
    if calc_type == "ts":
        for i in range(edge_index.shape[1]):
            edge_tuple = (
                nodes_list[edge_index[0, i].item()],
                nodes_list[edge_index[1, i].item()],
            )
...
def atoms_to_data(
    ...
    graph, surf_atoms, _ = atoms_to_pyggraph(
        structure,
        "int",
