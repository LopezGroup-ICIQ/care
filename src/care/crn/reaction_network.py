...
from care.crn.utilities import chemspace
...
class ReactionNetwork:
    """Chemical Reaction Network class for generating and analyzing chemical reaction networks.
    """

    def __init__(
            self,
            intermediates: Union[dict[str, Chem.Mol], dict[str, Atoms], dict[str, Intermediate]] = {},
            reactions: Union[list[list[str], list[str], str], list[ElementaryReaction]] = [],
        ...
        if isinstance(intermediates, dict):
            ...
            if all([isinstance(value, Chem.Mol) for value in intermediates.values()]) or all([isinstance(value, Atoms) for value in intermediates.values()]):
                self.intermediates = chemspace.gen_intermediates(intermediates)

            elif all([isinstance(value, Intermediate) for value in intermediates.values()]):
                self.intermediates = intermediates
            else:
                raise TypeError("Values of intermediates must be either Chem.Mol, Atoms or Intermediate")
        else:
            raise TypeError("intermediates must be a dictionary")
        
        if isinstance(reactions, list):
            if all([isinstance(reaction, list) for reaction in reactions]):
                self.reactions = self.gen_reactions(reactions, additional_rxns=additional_rxns)
            elif all([isinstance(reaction, ElementaryReaction) for reaction in reactions]):
    ...
    def gen_reactions(self, reactions: list, additional_rxns: bool):
        ...
        for reaction in reactions:
            reactant = self.intermediates[
                reaction[0][0]] 
            product1 = self.intermediates[
                reaction[1][0]] 

            if len(reaction[1]) == 2:
                product2 = self.intermediates[
                    reaction[1][1] ] 
                reaction_components = [[self.surface, reactant], [product1, product2]]
            else:
        ...
        ads_des_rxns = chemspace.gen_adsorption_reactions(self.intermediates, self.surface)
        rxns_list.extend(ads_des_rxns)

        if additional_rxns:  # Call here the desired functions to generate additional reactions
            rearrangement_rxns = chemspace.gen_rearrangement_reactions(self.intermediates)
            rxns_list.extend(rearrangement_rxns)

        return list(set(rxns_list))
