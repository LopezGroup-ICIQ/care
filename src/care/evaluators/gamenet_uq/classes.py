...
from care.evaluators.gamenet_uq.functions import get_graph_conversion_params, get_mean_std_from_model
...
class PreTrainedModel:
    ...
    def __repr__(self) -> str:
        string = (
            "Pretrained graph neural network for DFT ground state energy prediction."
        )
...
class EnsembleModel:
    ...
    def __repr__(self) -> str:
        string = "Ensemble of {} pretrained GNNs for DFT ground state energy prediction.".format(
            self.num_ensembles
        )
        string += "\nModel paths: {}".format(
            [osp.abspath(model.model_path) for model in self.models]
        )
        string += "\nNumber of parameters: {}".format(
            sum([model.num_parameters for model in self.models])
        )
        string += "\nModel size: {:.2f} MB".format(
            sum([model.size_all_mb for model in self.models])
        )
        return string
    ...
    def get_adsorption_energy(self, graph: Data) -> float:
        ...
        with torch.no_grad():
            ...
            std_adsorption_energy = (
                adsorption_energy[1] ** 2 + molecule_energy[1] ** 2
            ) ** 0.5
